...
# Evaluate the model
cm<-confusionMatrix(predictions, test_data$final_result)
...
sigmoid_data <- data.frame(Predictions = predictions, Probabilities = predicted_probs[, "Withdrawn"],
                           Actuals = as.numeric(test_data$final_result == "Withdrawn")) # Assuming "Withdrawn" is the positive class
...
# Make predictions on the test data
predictions <- predict(model, newdata = test_data, type = "prob")[, "Withdrawn"] # Extract probabilities for the "Withdrawn" class

# Compute the ROC curve and calculate AUC
roc_data <- roc(test_data$final_result, predictions)

# Visualize the ROC curve
plot.roc(roc_data, print.auc = TRUE, auc.polygon = TRUE, grid = TRUE,
         main = "ROC Curve", col = "steelblue")